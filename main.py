...
def send_sub_request(chat_id):
    ...
    bot.send_message(
        chat_id, 
        "⚠️ **Botdan to'liq foydalanish uchun quyidagi barcha homiy kanal va guruhlarga obuna bo'ling:**",
        parse_mode="Markdown", 
        reply_markup=markup
    )
...
def handle_menu(message):
    ...
    if text == "🎟 Promokod qo'shish" and user_id == ADMIN_ID:
        ...
        bot.send_message(message.chat.id, "Promokod va summasini yozing (Masalan: `BONUS 5000`):")
        return
...
def callback_handler(call):
    ...
    if data.startswith("cfg_free_") and user_id == ADMIN_ID:
        ...
        bot.send_message(call.message.chat.id, f"Tekin quti ({box_num}-quti) sovrinini yozing:")
        return
    ...
    if data.startswith("cfg_vip_") and user_id == ADMIN_ID:
        ...
        bot.answer_callback_query(call.id)
        bot.send_message(call.message.chat.id, f"VIP quti ({box_num}-quti) sovrinini yozing:")
        return
